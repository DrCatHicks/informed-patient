...
1. **Systematic reviews — cast a wide net, not just Cochrane**
   ...
   - `site:cochranelibrary.com [condition]` — Cochrane first. **If this returns no results, do not silently skip it.** Run a fallback query (`"Cochrane review" [condition]`) and report one of two things: (a) "I found a Cochrane review: [title, URL]" or (b) "I searched Cochrane and found no systematic review for this condition — this could suggest the condition is understudied or complex, and may require more individualized search to verify clinical consensus." The site: operator is not always honored by the search tool; the fallback ensures you've actually looked before concluding nothing exists.
   - `site:pubmed.ncbi.nlm.nih.gov [condition] systematic review` — PubMed indexes systematic reviews published in any peer-reviewed journal; a well-conducted review in JAMA or The Lancet is strong evidence
   - `site:nice.org.uk [condition]` — NICE (UK) produces high-quality evidence reviews as part of guideline development, often covering conditions Cochrane hasn't addressed
   - `site:effectivehealthcare.ahrq.gov [condition]` — AHRQ (US) commissions systematic reviews via Evidence-Based Practice Centers. **Do not silently skip this.** If the search returns no results, note it explicitly: "I searched AHRQ and found no review for this condition." If strong Cochrane coverage already exists and you're stopping early, still note the skip: "AHRQ not searched — Cochrane and guideline coverage was sufficient." Silent omission has been a recurring failure in testing.
   ...
   **If systematic reviews are absent:** Search PROSPERO (`site:crd.york.ac.uk/prospero [condition]`) — the international register of systematic reviews in progress. Finding a registered in-progress review is itself useful information: tell the user that research is underway but not yet published.
...
3. **Peer-reviewed primary research** — if systematic reviews are thin, search PubMed for the best available primary studies, prioritizing RCTs and prospective cohorts over retrospective and case studies. Search: `site:pubmed.ncbi.nlm.nih.gov [condition] [key symptoms] RCT` and `site:pubmed.ncbi.nlm.nih.gov [condition] cohort study`.

4. **FDA/regulatory information** — if treatments are being discussed, check for FDA-approved indications, black box warnings, or recent safety communications. This applies to drug classes as well as specific drug names. Search: `site:fda.gov [drug class or drug name condition]` (e.g., `site:fda.gov ACE inhibitors hypertension` or `site:fda.gov lisinopril`).
